...
/**getters and setters test.*/
  ...
  */
   @Test public void readLargeFileTest() throws IOException {
      ...
      Assert.assertTrue("test", team.generateReport().contains("Pat Jones"));
      Assert.assertTrue("test", team.generateReport().contains("Jackie Smith"));
      Assert.assertTrue("test", team.generateReport().contains("Jo Williams"));
      Assert.assertTrue("test", team.generateReport().contains("Sammi James"));
      Assert.assertEquals("get player Count", 4, team.getPlayerCount());
      Assert.assertEquals("get excluded count", 2, team.getExcludedCount());
      Assert.assertEquals("get ignored count", 0, team.getIgnoredCount());
      Assert.assertEquals("rosterread file", 
         team.getRoster()[0].getName(), "Pat Jones");
   }
   /**

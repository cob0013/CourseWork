...
//instance variables
   ...
   */
   ...
   /**
   *finds pp w shortest base edge.
   *@return ppWshortestBaseEdge 
   */
   public PentagonalPyramid findPentagonalPyramidWithShortestBaseEdge() {
      
      PentagonalPyramid output = null;
      if (numberOfPentagonalPyramids != 0) {
         double shortest = penPyramidList[0].getBaseEdge();
         output = penPyramidList[0];
         for (int i = 0; i < numberOfPentagonalPyramids; i++) {
            if (penPyramidList[i].getBaseEdge() < shortest) {
               output =  penPyramidList[i];
            }
   ...
   /**
   *finds pp w longest base edge.
   *@return ppWLongestBaseEdge 
   */
   public PentagonalPyramid findPentagonalPyramidWithLongestBaseEdge() {
      PentagonalPyramid output = null;
      if (numberOfPentagonalPyramids != 0) {
         output = penPyramidList[0];
         double longest = penPyramidList[0].getBaseEdge();
         for (int i = 1; i < numberOfPentagonalPyramids; i++) {
            output = penPyramidList[0];
            if (penPyramidList[i].getBaseEdge() > longest) {
               output = penPyramidList[i];
            }
         }
      }
   ...
   /**
   *finds pp w smallest volume.
   *@return ppWSmallestVolume 
   */
   public PentagonalPyramid findPentagonalPyramidWithSmallestVolume() {
      PentagonalPyramid output = null;
      if (numberOfPentagonalPyramids != 0) {
         output = penPyramidList[0];
         double smallestVolume = penPyramidList[0].volume();
         for (int i = 1; i < numberOfPentagonalPyramids; i++) {
            if (penPyramidList[i].volume() < smallestVolume) {
               output = penPyramidList[i];
            }
         }
      }
   ...
   /**
   *finds pp w largest volume.
   *@return ppWLargestVolume 
   */
   public PentagonalPyramid findPentagonalPyramidWithLargestVolume() {
      PentagonalPyramid output = null;
      if (numberOfPentagonalPyramids != 0) {
         output = penPyramidList[0];
         double largestVolume = penPyramidList[0].volume();
         for (int i = 1; i < numberOfPentagonalPyramids; i++) {
            if (penPyramidList[i].volume() > largestVolume) {
               output = penPyramidList[i];
            }
         }
      }
